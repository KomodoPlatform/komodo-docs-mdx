export const title = "Test komodo source from jl777 branch";
export const description = "This guide describes a step-by-step process to test komodo source from jl777 branch to make sure all the smartchains sync from scratch properly.";

# Test komodo source from jl777 branch to make sure all the smartchains sync from scratch properly

## Purpose

The purpose of this test is to check if the komodo source from jl777 branch has no issues compiling, all smartchains starts and syncs fully from scratch, no stuck chains. You can use this guide for any branch you want to test, just change the branch name in clone and compile section.

<Note>
  This doc is similar to compiling komodo with an additional `verify` script. Please make sure you are not testing this in a node with funds. This doc is intended for test nodes/computer. If you have funds in your smartchains please backup your wallet and private key. The information in this doc may change in the future to update.
</Note>
...
3.  Create komodo.conf file and clear all smartchains dir from .komodo data dir
4.  Start Smartchains using ./assetchains.old and let them sync (it may take a while)
5.  Verify the smartchains
...
### 3. Create `komodo.conf` & clear existing smartchains database and files
...
Remove existing smartchain block database and files
