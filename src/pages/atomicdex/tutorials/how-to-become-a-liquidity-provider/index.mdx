...
We recommend that the user [have a Virtual Private Server (VPS)](https://developers.komodoplatform.com/smart-chains/tutorials/basic-environment-setup-for-linux-vps) with _at least_ the following specifications.
...
If you decide to build from source, please follow the [Build AtomicDEX API from source guide](how-to-compile-mm2-from-source)
...
Replace the text `"MM2_RPC_PASSWORD"` with your RPC user password.
...
Replace the text `MM2_RPC_PASSWORD` with the same password used in the `start.sh` file.
