...
By default, collected metrics are recorded to a log file every five minutes, but this value can be defined with the `metrics` parameter in a user's [MM2.json file](/atomicdex/setup/configure-mm2-json).
...
Prometheus scrapes metrics using an HTTP pull model. To provide Prometheus with the ability to scrape the metrics at [localhost:9001](http://localhost:9001), you need to add the `prometheusport` parameter to your [MM2.json file](/atomicdex/setup/configure-mm2-json):

```
"prometheusport": 9001
```

You can also define your username and password for Prometheus to enforce basic authorization security with the `prometheus_credentials` in your [MM2.json file](/atomicdex/setup/configure-mm2-json). Note that this additional argument is NOT necessary.
