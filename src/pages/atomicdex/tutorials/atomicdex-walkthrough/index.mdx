...
| userhome     | <span>{'/${HOME#"/"}'}</span> | the path to your home, called from your environment variables and entered as a regular expression                                                                                  |
...
<Note>
  The `userpass` environment variable will remain in our terminal's memory until
  we close the session. When we open up a new session later, we need to create
  the `userpass` environment variable again. Later, a GUI designer can create
  functionality to perform this automatically.
</Note>
