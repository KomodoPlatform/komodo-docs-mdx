...
Create a new file in the `~/atomicDEX-API/target/debug` folder called `userpass`, then enter the following text using same the `rpc_password` as is in your MM2.json file or runtime configuration string:
...
While we did set up the `coins` file for wach coin's configuration properties earlier, the AtomicDEX-API doesn't to connect to their blockchain networks until we send a [coin activation request](pages/atomicdex/api/legacy/coin_activation).
...
In a minute or so a few MARTY coins will arrive in your address. You can [check your balance](pages/atomicdex/api/legacy/my_balance) with the request below:
...
To see the orders listed on the AtomicDEX orderbook, use the [`orderbook`](pages/atomicdex/api/v20/orderbook) method:
...
Let's create a DOC/MARTY [`buy`](pages/atomicdex/api/legacy/buy) order
...
```bash· 2023-05-06 22:05:31 +0800 [] lp_ordermatch:3094] Entering the taker_swap_loop DOC/MARTY with uuid: a8122ae2-2f20-4c19-b2f7-c4fa53d9e52a
...
When you see that last line, you know that your swap went through! Let's check by checking our DOC [balance](pages/atomicdex/api/legacy/my_balance) again.
...
Once you are finished using the AtomicDEX-API, you can exit using the [`stop`](pages/atomicdex/api/legacy/stop) method:
