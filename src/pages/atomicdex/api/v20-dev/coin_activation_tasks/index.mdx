export const title = "KomoDeFi: Coin Activation Tasks";
export const description = "The methods in this document allow activation of UTXO coins and specifically QTUM coins.";
...
UTXO coins can be activated using this method. For QTUM coins, refer to [task::enable\_qtum::init](#task-enable-qtum-init).
Though you can use this method (or the legacy alternatives) for non-HD wallets, when using a HD wallet this method must be used.
...
| activation\_params              | object  | A standard [ActivationParams](/atomicdex/api/v20#activation-params) object.                                                                                                                                      |
...
| forget\_if\_finished | boolean | If `false`, will return the final response for completed tasks. Optional, defaults to `true`. |
...
#### Response (ready, successful, HD wallet mode)

| Parameter                | Type            | Description                                                                      |
| ------------------------ | --------------- | -------------------------------------------------------------------------------- |
| status                   | string          | A short indication of how the requested process is progressing.                  |
| details                  | object          | A simple object containing information as detailed in the items below.           |
| details.ticker           | string          | The ticker of the coin being activated                                           |
| details.current\_block   | integer         | The block height of the coin being activated                                     |
| details.wallet\_balance  | object          | A standard [`WalletBalanceInfo`](/atomicdex/api/v20#wallet-balance-info) object. |
...
*   `HwError` - **This is the most important error type.** Unlike other error types, `HwError` requires the GUI / User to check the details in `error_data` field to know which action is required. View the [HwError error type details](/atomicdex/api/v20-dev/trezor_initialisation/#Details_for_HwError_error_type) for more info.
...
QTUM coins are activated using this method. For UTXO coins, refer to [task::enable\_utxo::init](#task-enable-utxo-init)
...
| .scan\_policy                   | string  | Whether or not to scan for new addresses. Select from `do_not_scan`, `scan_if_new_wallet` or `scan`. Note that `scan` will result in multple requests to the KomoDeFi-API.                                      |
...
The response formats for this method are the same as the [task::enable\_utxo::status](#task-enable-utxo-status) responses.
