...
The methods below prepare your Trezor device for use within the KomoDeFi API. Once completed, you can authenticate using your PIN or phrase with [task::init\_trezor::user\_action](#task-init-trezor-user-action).
...
<CollapsibleSection expandedText="Hide Response" collapsedText="Show Response">
  ...
  *   `HwError` - **This is the most important error type.** Unlike other error types, `HwError` requires the GUI / User to check the details in `error_data` field to know which action is required. View the [HwError error type details](/atomicdex/api/v20-dev/hd_wallets_overview/#details-for-hw-error-error-type) for more info.
</CollapsibleSection>
