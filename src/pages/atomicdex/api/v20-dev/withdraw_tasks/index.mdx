...
The `task::withdraw::init` method generates, signs, and returns a transaction that transfers the `amount` of `coin` to the address indicated in the `to` argument. The status of this method can be queried via the [withdraw\_status](/atomicdex/api/v20-dev/withdraw_tasks/#task-withdraw-status) method.
...
To cancel the transaction generation, use the [withdraw\_cancel](/atomicdex/api/v20-dev/withdraw_tasks/#task-withdraw-cancel) method.
...
| from                  | string           | Optional, used only for transactions using a hardware wallet. For more information, see the [Trezor Integration guide](/atomicdex/api/v20-dev/trezor_initialisation/)                                                                                                                                       |
