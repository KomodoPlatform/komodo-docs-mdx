...
| tokens_params               | array of objects | A list of standard [TokensRequest](src/pages/atomicdex/common/structures#TokensRequest) objects.                                                                                                                                                                            |
...
| balance          | object           | Only returned when `get_balances` is `true`. A standard [BalanceInfos](src/pages/atomicdex/common/structures#BalanceInfos) object.                              |
| tokens_balances  | array of objects | Only returned when `get_balances` is `true`. A list of standard [addressInfos](src/pages/atomicdex/common/structures#addressInfos) objects, one for each token. |
| tokens_tickers   | array            | Only returned when `get_balances` is `false`. A list of each token which was activated.                                                                         |


### 📌 Examples

#### Request with `get_balances` set to `true`

<CodeGroup title="" tag="POST" label="enable_tendermint_with_assets">
