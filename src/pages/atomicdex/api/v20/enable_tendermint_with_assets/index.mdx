...
| tokens\_balances | array of objects | Only returned when `get_balances` is `true`. A list of standard [addressInfos](/atomicdex/api/common_structures/#address-infos) objects, one for each token. |
