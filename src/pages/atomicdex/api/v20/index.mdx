...
### ConfirmationTargets
<DevComment>
  This section needs descriptions for each of the parameters.
</DevComment>
...
<DevComment>
  What's the default for `allow_outbound_0conf`?
</DevComment>

| Parameter                          | Type    | Description                                                              |
|------------------------------------|---------|--------------------------------------------------------------------------|
| allow_outbound_0conf               | boolean | Optional. When setting an outbound channel, it can be used straight away [without waiting](https://docs.rs/lightning/latest/lightning/util/config/struct.ChannelHandshakeLimits.html#structfield.trust_own_funding_0conf) for any on-chain confirmations. |
...
| trust_own_funding_0conf            | boolean | Optional, defaults to `true`. Confirmations we will wait for before considering an inbound channel locked in.    |
...
<DevComment>
`color` default value may change to use one from the [KomodoPlatform style guide](https://komodoplatform.com/assets/dl/Komodo_Brand_Assets.zip)
</DevComment>
...
| commit_upfront_shutdown_pubkey     | boolean | Optional, defaults to `true`. If `true`, we commit to an upfront shutdown_pubkey at channel open.                                                      |
...
<DevComment>
It would be good to better explain (or link to somthing that explains) what it means to "commit to an upfront shutdown_pubkey at channel open"
</DevComment>
...
<DevComment>
Needs default values for `their_channel_reserve_sats`.
</DevComment>
