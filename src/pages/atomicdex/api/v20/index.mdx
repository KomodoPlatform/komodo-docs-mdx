...
| from_date              | string  | Optional. A UTC date string in the format `YYYY-MM-DD HH:MM:SS.SSS`. Only transfers after this date are included in the response.   |
| to_date                | string  | Optional. A UTC date string in the format `YYYY-MM-DD HH:MM:SS.SSS`. Only transfers before this date are included in the response.  |
...
| token_name             | string  | Optional. Name of the token.                                                                                                      |
...
| animation_url          | string  | Optional. Url to an animationto be displaye instead of a static image.                                                            |
...
| withdraw_data | object           | A standard [WithdrawDataErc721](/atomicdex/api/v20#withdraw-data-erc721) or [WithdrawDataErc1155](/atomicdex/api/v20#withdraw-data-erc155) object.                                       |
...
Due to this difference, the `amount` and `max` fields are only used the when `type` value in `WithdrawType` is `withdraw_erc1155`.
</Note>
