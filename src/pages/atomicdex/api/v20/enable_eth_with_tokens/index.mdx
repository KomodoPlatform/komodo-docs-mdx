...
| erc20\_tokens\_requests     | array of objects | A list of standard [TokensRequest](/atomicdex/api/v20#tokens-request) objects.                                                                                                                                                                                                                                                                                                |
| gas\_station\_url           | string           | Optional, for ETH/ERC20 and other gas model chains. Defaults to [eth\_gasPrice RPC API](https://github.com/ethereum/wiki/wiki/JSON-RPC#eth_gasprice). URL of [ETH gas station API](https://docs.ethgasstation.info/) When this parameter is set, the AtomicDEX API will request the current gas price from Station for new transactions, and this often results in lower fees |
...
| rpc\_mode                   | string           | Optional, defaults to `Http`, value can be `Metamask` when the AtomicDEX-API is built targeting `wasm`                                                                                                                                                                                                                                                                        |
...
| eth\_addresses\_infos   | object  | A standard [addressInfos](/atomicdex/api/v20#address-infos) object. Note: the structure may vary based on the value of the `get_balances` parameter. |
| erc20\_addresses\_infos | object  | A standard [addressInfos](/atomicdex/api/v20#address-infos) object. Note: the structure may vary based on the value of the `get_balances` parameter. |
...
<CollapsibleSection expandedText="Hide Errors" collapsedText="Show Errors">
  ...
  In this case, you need to [disable](/atomicdex/api/legacy/disable_coin) the platform coin and try again.
