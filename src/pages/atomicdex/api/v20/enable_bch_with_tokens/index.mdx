...
| utxo\_merge\_params      | object           | A standard [UtxoMergeParams](/atomicdex/api/common_structures/activation/#utxo-merge-params) object. Used to reduce a wallet's UTXO count in cases where it is causing significantly slower RPC responses.                                  |
...
| bch\_addresses\_infos | object  | A standard [AddressInfo](/atomicdex/api/common_structures/#address-info) object. Note: the structure may vary based on the value of the `get_balances` parameter. |
| slp\_addresses\_infos | object  | A standard [AddressInfo](/atomicdex/api/common_structures/#address-info) object. Note: the structure may vary based on the value of the `get_balances` parameter. |
