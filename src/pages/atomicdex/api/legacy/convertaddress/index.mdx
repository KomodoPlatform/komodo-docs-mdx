export const title = "AtomicDEX Method: Convert Address";
...
| to\_address\_format         | object        | address format to which the input address should be converted                                                                                               |
| to\_address\_format.format  | string (enum) | address format to which the input address should be converted, possible values: `mixedcase` for ETH/ERC20 coins; `cashaddress` or `standard` for UTXO coins |
| to\_address\_format.network | string (enum) | network prefix for `cashaddress` format. Possible values: `bitcoincash` for BCH mainnet; `bchtest` for BCH testnet; `bchreg` for BCH regtest                |
