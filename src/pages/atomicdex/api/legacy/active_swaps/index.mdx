...
export const description = "The active_swaps method returns all the swaps that are currently running on the AtomicDEX API node.";
...
| -------------- | ---- | ----------------------------------------------------------------- |
| include\_status | bool | whether to include swap statuses in response; defaults to `false` |
...
| result.statuses | object (map)     | the `uuid -> swap status` map of currently running swaps; `null` if include\_status is false in the request |
