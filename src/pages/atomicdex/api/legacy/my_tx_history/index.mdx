...
export const description = "The my_tx_history method returns the blockchain transactions involving the AtomicDEX API node's coin address.";
...
The coin that is used must have `tx_history` set to true in its [enable](/atomicdex/api/legacy/enable.html) or [electrum](/atomicdex/api/legacy/electrum.html) call.
