export const title = "KomoDeFi: Coin Activation";
...
<Note type="warning">
  The KomoDeFi API requires an `mm2` parameter to be set for each `coin`. This can be added to the enable/electrum command, or defined in your [coins file](/atomicdex/tutorials/atomicdex-walkthrough/#setting-up-the-coin-list).
...
If you are running a UTXO based coin daemon locally and the blockchain is synchronised, you can connect the local daemon to the KomoDeFi API by using the `enable` method, though some additional configuration is required. The KomoDeFi API requires the following options to be added to the native chain's [.conf file](http://bitcoincoredocs.com/bitcoin-conf.html).
...
<Note>
  The KomoDeFi API node's coin address needs to be imported manually into the coin daemon using the [importaddress](/smart-chains/api/wallet/#importaddress) method.
</Note>
...
| servers.protocol                         | string (optional, defaults to `TCP`)    | Transport protocol used by KomoDeFi API to connect to the electrum server (`TCP` or `SSL`)                                                                                                                                                |
...
| mm2                                      | integer                                 | Required if not set in `coins` file. Informs the KomoDeFi API whether or not the coin is expected to function. Accepted values are `0` or `1`                                                                                             |
| tx\_history                              | bool                                    | If `true` the KomoDeFi API will preload trasaction history as a background process. Must be set to `true` to use the [my\_tx\_history](/atomicdex/api/legacy/my_tx_history/#my-tx-history) method                                         |
| required\_confirmations                  | integer (optional, defaults to `3`)     | Number of confirmations for the KomoDeFi API to wait during the transaction steps of an atomic swap.                                                                                                                                      |
...
| required\_confirmations | number            | the number of transaction confirmations for which the KomoDeFi API must wait during the atomic swap process                                                    |
...
#### UTXO coin activation {{title : 'Electrum Method', label : 'electrum', tag : 'legacy'}}
...
#### UTXO coin activation with `required_confirmations`, `requires_notarization` and `mm2` arguments {{title : 'Electrum Method', label : 'electrum', tag : 'legacy'}}
...
#### QTUM/QRC20 coin activation {{title : 'Electrum Method', label : 'electrum', tag : 'legacy'}}
...
| swap\_contract\_address     | string (required for QRC20 only)                                     | address of etomic swap smart contract                                                                                                                                                                                                                                                                                                            |
| fallback\_swap\_contract    | string (required for QRC20 only)                                     | address of backup etomic swap smart contract                                                                                                                                                                                                                                                                                                     |
| gas\_station\_url           | string (optional for ETH/ERC20 and other gas model chains)           | url of [ETH gas station API](https://docs.ethgasstation.info/); The KomoDeFi API uses [eth\_gasPrice RPC API](https://github.com/ethereum/wiki/wiki/JSON-RPC#eth_gasprice) by default; when this parameter is set, the KomoDeFi API will request the current gas price from Station for new transactions, and this often results in lower fees |
...
| mm2                         | integer                                                              | Required if not set in `coins` file. Informs the KomoDeFi API whether or not the coin is expected to function. Accepted values are `0` or `1`                                                                                                                                                                                                   |
| tx\_history                 | bool                                                                 | If `true` the KomoDeFi API will preload transaction history as a background process. Must be set to `true` to use the [my\_tx\_history](/atomicdex/api/legacy/my_tx_history/#my-tx-history) method                                                                                                                                              |
| required\_confirmations     | integer (optional, defaults to `3`)                                  | Number of confirmations for the KomoDeFi API to wait during the transaction steps of an atomic swap.                                                                                                                                                                                                                                            |
| requires\_notarization      | boolean (optional, defaults to `false`)                              | If `true`, coins protected by [Komodo Platform's dPoW security](https://satindergrewal.medium.com/delayed-proof-of-work-explained-9a74250dbb86) will wait for a notarization before progressing to the next atomic swap transactions step.                                                                                                       |
...
| required\_confirmations | number            | KomoDeFi API will wait for the this number of coin's transaction confirmations during the swap                    |
...
#### Native UTXO coin activation {{title : 'Enable Method', label : 'enable', tag : 'legacy'}}
...
#### EVM coin activation with gas\_station\_url and policy {{title : 'Enable Method', label : 'enable', tag : 'legacy'}}
...
#### Polygon (MATIC) and PLG20 activation {{title : 'Enable Method', label : 'enable', tag : 'legacy'}}
...
#### Binance Coin (BNB) and BEP20 activation {{title : 'Enable Method', label : 'enable', tag : 'legacy'}}
...
For enabling Z coins, refer to the [Z coin tasks](/atomicdex/api/v20-dev/task_enable_z_coin/#init) in the v2.0 Dev API.
