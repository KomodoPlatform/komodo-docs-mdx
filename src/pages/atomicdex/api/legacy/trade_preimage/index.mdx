...
# trade\_preimage {{title : '', label : 'trade_preimage', tag : 'deprecated'}}
...
<Note>
  This function is deprecated. Please consider using [trade\_preimage v2.0](/atomicdex/api/v20/trade_preimage/) instead.
</Note>
...
| result.total\_fees               | array of `TotalFeeInfo` objects      | each element is a sum of fees required to be paid from user's balance of corresponding `ExtendedFeeInfo.coin`; the elements are unique by coin |
