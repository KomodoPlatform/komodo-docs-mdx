...
<Note>
Using the same seed phrase with / without setting the `hd_account_id` value will result in different addresses being generated.
For example, when using the seed phrase `february soldier message acid member jump shadow walk novel impose puppy tornado`:
- If the `hd_account_id` value is set to `0` in MM2.json, the first address in the wallet returns the address `0x26cE5D1cD4CC673a1b0b980371C7c862B9503452` for ETH
- If the `hd_account_id` value is not set, the wallet will return the address `0x8c40a6E127c7a13e26ce95deA88354C3fb134580` for ETH
</Note>
