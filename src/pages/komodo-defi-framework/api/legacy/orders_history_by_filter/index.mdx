...
# Komodo DeFi Framework Method: Orders History By Filter
...
| order\_type      | string  |     ✗    | A standard [OrderTypeEnum](/komodo-defi-framework/api/common_structures/enums/#order-type-enum) enum. Return only orders that match the `order_type`     |
| rel              | string  |     ✗    | Return only orders that match the `order.rel = rel` condition                                                                                            |
| status           | string  |     ✗    | A standard [OrderStatusEnum](/komodo-defi-framework/api/common_structures/enums/#order-status-enum) enum. Return only orders that match the `status`     |
