...
| gasfree\_address     | string           | Optional. TRON GasFree coins/tokens only: the deterministic GasFree custody address for this address. Present only when the coin was activated with a [TronGaslessProviderConfig](/komodo-defi-framework/api/common_structures/activation/#tron-gasless-provider-config); omitted otherwise. See [TRON GasFree transfers](/komodo-defi-framework/tutorials/tron-gasfree/). |
