...
| sync\_params              | integer or string |     ✗    | `2 days ago` | ZHTLC coins only. Optional, defaults to two days ago. Defines where to start scanning blockchain data upon initial activation. Options: `"earliest"` (the coin's sapling\_activation\_height), `height` (a specific block height) or `date` (a unix timestamp). |
...
| requires\_notarization       | boolean |     ✗    |        `false`       | Optional, defaults to `false`. For [dPoW](https://komodoplatform.com/en/blog/dpow-demystified/) protected coins, a `true` value will wait for transactions to be notarised when doing swaps. Overrides value if set in `coins` file.                                          |
...
| scan\_blocks\_per\_iteration | integer |     ✗    |        `1000`        | ZHTLC coins only. Sets the number of scanned blocks per iteration during `BuildingWalletDb` state. Optional, default value is 1000.                                                                                                                                           |
| scan\_interval\_ms           | integer |     ✗    |          `0`         | ZHTLC coins only. Sets the interval in milliseconds between iterations of `BuildingWalletDb` state. Optional, default value is 0.                                                                                                                                             |
...
| scan\_policy                 | string  |     ✗    | `scan_if_new_wallet` | Optional, HD wallets only. Whether or not to scan for new addresses. Select from `do_not_scan`, `scan_if_new_wallet` or `scan`. Defaults to `scan_if_new_wallet`. Note that `scan` will result in multple requests to the Komodo DeFi API and may take some time to complete. |
| gap\_limit                   | integer |     ✗    |         `20`         | Optional, HD wallets only. The max number of empty addresses in a row. Transactions sent to an address outside the `gap_limit`, will not be identified when scanning. Defaults to `20`.                                                                                       |
...
| get\_balances                | boolean |     ✗    |        `true`        | Optional, defaults to `true`. If `false`, coin and token balances will not be returned in the response, and the response will be returned more quickly.                                                                                                                       |
...
| protocol                    | string  |     ✗    |  `TCP`  | Optional, defaults to `TCP`. Transport protocol used to connect to the server. Options: `TCP` or `SSL`                                                            |
| disable\_cert\_verification | boolean |     ✗    | `false` | Optional, defaults to `false`. If `true`, this disables server SSL/TLS certificate verification (e.g. for self-signed certificates). <b>Use at your own risk!</b> |
...
| Parameter   | Type    | Required |   Default  | Description                                                                              |
| ----------- | ------- | :------: | :--------: | ---------------------------------------------------------------------------------------- |
| account\_id | integer |     ✗    |     `0`    | Optional, defaults to `0`. Used as a layer of separation or hierarchy.                   |
| chain       | string  |     ✗    | `External` | Optional. Accepted values are `External` (0) and `Internal` (1). Defaults to `External`. |
| address\_id | integer |     ✗    |     `0`    | Optional, defaults to `0`. Used as a layer of separation or hierarchy.                   |
...
| komodo\_auth | boolean |     ✗    | `false` | Optional, defaults to `false`. Must be set to `true` to access RPC nodes run behind [komodo-defi-proxy](https://github.com/KomodoPlatform/komodo-defi-proxy) |
...
| required\_confirmations | integer |     ✗    |   `3`   | How many confirmations to wait during the transaction steps of an atomic swap. Overwrites value in coins file; defaults to `3` |
