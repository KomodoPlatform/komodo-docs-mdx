...
<Note>
  TRON withdrawals do not support manual fee parameters. Fees are auto-estimated based on bandwidth and energy consumption. Omit the `fee` field for TRON/TRC20 withdrawals.
</Note>
...
The `GaslessWithdrawOptions` object supplies caller constraints for a gasless TRON TRC20 withdrawal. It is passed as the `gasless` field of a [withdraw](/komodo-defi-framework/api/v20/wallet/tx/withdraw/) request when `fee_method` is `gasless`. All fields are optional. See [TRON GasFree transfers](/komodo-defi-framework/tutorials/tron-gasfree/) for the full flow.

<CompactTable source="common-structures/wallet.GaslessWithdrawOptions" />
...
<CollapsibleSection expandedText="Hide Examples" collapsedText="Show Examples">
  ...
  #### Example of Tron type (native TRX transfer to activated address)
  ...
  #### Example of Tron type (native TRX transfer to unactivated address)

  When sending TRX to an address that has never been activated on the Tron blockchain,
  an `account_creation_fee` is included. This fee is burned by the network to create
  the destination account. The fee is charged to the sender on top of the transfer
  amount — the recipient receives the full amount.
  ...
  <Note>
    The `account_creation_fee` field is only present for native TRX transfers to
    unactivated addresses. It is omitted (not `null`) for transfers to existing
    addresses and for all TRC20 token transfers.
  </Note>

  #### Example of Tron type (TRC20 token transfer)
  ...
  #### Example of TronGasless type (gasless TRC20 transfer via GasFree)

  The fee is paid in the transferred token, not in TRX. An `activation_fee` appears only when the GasFree address must be activated on its first transfer.
