...
This section provides an overview of Lightning Network Initialization methods. Please see the following pages for detailed documentation on each method:

- [Initialize Lightning](./init/)
- [Initialization Status](./status/)
- [Cancel Initialization](./cancel/)

## Initialize Lightning {{label : 'task::enable_lightning::init', tag : 'API-v2'}}

The `task::enable_lightning::init` requests a task to run a lightning node. Use the returned `task_id` as an input to check the status of the lightning node (i.e., running or still initiating). An error will be returned if a lightning node was already running for the requested ticker.
...
<Note>
  In the above response, spendable will always be 0 since the balance is unspendable until connections with lightning channels counterparties are established. Using the [my\_balance](/komodo-defi-framework/api/legacy/my_balance/) method after the coin is activated will get the spendable balance depending on how many channel counterparties are online. For exact channel balances and which channels are usable, use [lightning::channels::list\_open\_channels\_by\_filter](/komodo-defi-framework/api/v20/lightning/channels/#list-open-channels-by-filter).
</Note>
