...
| account\_index  | number |     ✓    |    –    | BIP-44 account number to inspect (usually `0`).                                                                                   |
...
| paging\_options | object |     ✗    |    –    | Either `{ "from_id": <address_id> }` **or** `{ "page_number": <n> }`. If omitted, the first page (`page_number = 1`) is returned. |
...
<DevComment>
  ...
  https://github.com/KomodoPlatform/komodo-defi-framework/issues/2511
</DevComment>
