...
| gasless              | object           | TRON only. Optional. A standard [GaslessWithdrawOptions](/komodo-defi-framework/api/common_structures/wallet/#gasless-withdraw-options) object constraining the gasless transfer (max fee, deadline). Only valid together with `fee_method: "gasless"`.                                                                                                        |
...
<Note>
  The fee is paid in the token: `spent_by_me` (and `my_balance_change`) include the token-denominated provider fee (`total_token_fee`) on top of the transfer amount — in the example above, 5 USDT transferred + 2 USDT fee = 7 USDT spent. Track the submitted transfer by polling [gasless::trace\_status](/komodo-defi-framework/api/v20/wallet/gasless_trace_status/) with the returned `trace_id`.
</Note>
...
A gasless withdrawal that cannot be served always returns an error — there is no automatic fallback to a native (TRX-paid) transfer. These errors arrive with `error_type: "Gasless"` and the specific variant nested in `error_data`:
