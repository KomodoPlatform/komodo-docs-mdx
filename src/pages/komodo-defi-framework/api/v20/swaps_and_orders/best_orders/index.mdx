...
# Komodo DeFi Framework Method: Best Orders
...
| exclude\_mine | bool   |     ✗    | `false` | If `true`, the users orders will be excluded from the response.                                                                              |
...
<CodeGroup title="Best Orders by Number" tag="POST" label="best_orders" mm2MethodDecorate="true">
  ```json
  {
...
<CollapsibleSection expandedText="Hide Best Orders by Number Response" collapsedText="Show Best Orders by Number Response">
...
<CodeGroup title="Best Orders by Number (Exclude Own)" tag="POST" label="best_orders" mm2MethodDecorate="true">
  ```json
  {
    "userpass": "RPC_UserP@SSW0RD",
    "method": "best_orders",
...
<CollapsibleSection expandedText="Hide Best Orders by Number (Exclude Own) Response" collapsedText="Show Best Orders by Number (Exclude Own) Response">
  #### Response (by number - success)
...
<CodeGroup title="Best Orders by Volume" tag="POST" label="best_orders" mm2MethodDecorate="true">
  ```json
  {
    "userpass": "RPC_UserP@SSW0RD",
    "method": "best_orders",
...
<CollapsibleSection expandedText="Hide Best Orders by Volume Response" collapsedText="Show Best Orders by Volume Response">
  ```json
  {
    "mmrpc": "2.0",
    "result": {
...
<CollapsibleSection expandedText="Hide Error Responses" collapsedText="Show Error Responses">
  ...
  Returned if the request type or action is invalid (e.g., `number` value must be integer, or type/action is not allowed).

  ##### CoinIsWalletOnly

  Returned if the coin is wallet only and cannot be traded.

  ##### P2PError

  Returned if there is a connection problem.
</CollapsibleSection>
