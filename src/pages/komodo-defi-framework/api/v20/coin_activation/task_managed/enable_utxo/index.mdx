...
<CodeGroup title="task::enable_utxo::init" tag="POST" label="task::enable_utxo::init" mm2MethodDecorate="true">
  ```json
  {
...
| forget\_if\_finished | boolean |     ✗    |  `true` | If `false`, will return final response for completed tasks. Optional, defaults to `true`. |

#### Command

<CodeGroup title="task::enable_utxo::status" tag="POST" label="task::enable_utxo::status" mm2MethodDecorate="true">
  ```json
  {
    "userpass": "RPC_UserP@SSW0RD",
    "mmrpc": "2.0",
