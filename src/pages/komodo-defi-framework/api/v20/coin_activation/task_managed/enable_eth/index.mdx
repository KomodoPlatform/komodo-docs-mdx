export const title = "Komodo DeFi Framework Method: ETH/EVM/TRON Activation Tasks";
export const description =
  "The methods in this document demonstrate the task managed activation of ETH/EVM and TRON coins and tokens.";

import trezorpin from "@/public/images/docs/api-images/trezor_pin.png";

# Task: ETH/EVM/TRON Activation

## task::enable\_eth::init {{label : 'task::enable_eth::init', tag : 'API-v2'}}

Use this method for task managed activation of ETH/EVM and TRON coins & tokens. This includes TRX (wallet-only) and TRC20 tokens. Refer to the [task managed activation overview](/komodo-defi-framework/api/v20/coin_activation/task_managed/) for activation of other coin types.

<Note>
  For TRON, gasless TRC20 transfers are enabled exactly as with [enable\_eth\_with\_tokens](/komodo-defi-framework/api/v20/coin_activation/enable_eth_with_tokens/): supply a `tron_gasless_provider` (a standard [TronGaslessProviderConfig](/komodo-defi-framework/api/common_structures/activation/#tron-gasless-provider-config)) and an optional per-token `gasless` constraint. See [TRON GasFree transfers](/komodo-defi-framework/tutorials/tron-gasfree/).
</Note>
