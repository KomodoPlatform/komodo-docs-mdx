export const title = "task::enable_bch::status";
export const description = "Query the status of BCH/SLP activation task.";

# Komodo DeFi Framework Method: task::enable\_bch::status
...
Possible `status` values while activation is in progress (see [EthActivationStatusEnum](/komodo-defi-framework/api/common_structures/enums/#eth-activation-status-enum)):
