...
To enable [gasless TRC20 transfers](/komodo-defi-framework/tutorials/tron-gasfree/), pass a `tron_gasless_provider` (a standard [TronGaslessProviderConfig](/komodo-defi-framework/api/common_structures/activation/#tron-gasless-provider-config)) when activating the TRON platform coin, and an optional per-token `gasless` constraint on each TRC20 [TokensRequest](/komodo-defi-framework/api/common_structures/activation/#tokens-request). Once activated, the response includes each address's `gasfree_address`, and gasless withdrawals become available through the [withdraw](/komodo-defi-framework/api/v20/wallet/tx/withdraw/) `fee_method: "gasless"` option.
