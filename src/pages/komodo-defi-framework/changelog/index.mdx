...
#### 🔗 TRON Support (Wallet-Only)

*   **feat(tron): add TRON wallet support** [PR #2714](https://github.com/KomodoPlatform/komodo-defi-framework/pull/2714): Added wallet-only support for TRON (TRX) and TRC20 tokens. Users can activate TRX via `enable_eth_with_tokens` or `task::enable_eth::init`, query balances, and perform withdrawals (both TRX and TRC20). TRON uses HTTP API nodes and Base58Check addresses (`T...` format). HD wallet derivation follows BIP44 with SLIP-44 coin type 195 (`m/44'/195'`). Atomic swaps are not yet supported for TRON-based coins.
*   **feat(tron): gasFree gasless TRC20 transfers** [PR #XXXX](https://github.com/KomodoPlatform/komodo-defi-framework/pull/XXXX): Added gasless TRC20 transfers on TRON via GasFree, letting users send TRC20 tokens (e.g. USDT) without holding TRX — the fee is paid in the transferred token. Activate a TRON platform coin with a [`tron_gasless_provider`](/komodo-defi-framework/api/common_structures/activation/#tron-gasless-provider-config) (direct GasFree API or Komodo proxy), withdraw with `fee_method: "gasless"`, submit the signed authorization via `send_raw_transaction`, and track status with [`gasless::trace_status`](/komodo-defi-framework/api/v20/wallet/gasless_trace_status/) or [`stream::gasless_trace::enable`](/komodo-defi-framework/api/v20/streaming/gasless_trace_enable/). The activation and balance responses surface each address's GasFree custody address (`gasfree_address`). See [TRON GasFree transfers](/komodo-defi-framework/tutorials/tron-gasfree/).
