...
Now that you have the AtomicDEX API [installed](/komodo-defi-framework/setup/) you are ready for your first atomic swap!
...
While we did set up the `coins` file for wach coin's configuration properties earlier, the AtomicDEX-API doesn't to connect to their blockchain networks until we send a [coin activation request](/komodo-defi-framework/api/legacy/coin_activation/).
...
To see the orders listed on the AtomicDEX orderbook, use the [`orderbook`](/komodo-defi-framework/api/legacy/orderbook/) method:
...
Once you are finished using the AtomicDEX-API, you can exit using the [`stop`](/komodo-defi-framework/api/legacy/stop/) method:
