...
1.  **Activate** the TRON platform coin with a [`tron_gasless_provider`](/komodo-defi-framework/api/common_structures/activation/#tron-gasless-provider-config), and optionally a per-token `gasless` constraint. Activation returns each address's `gasfree_address` (see [enable\_eth\_with\_tokens](/komodo-defi-framework/api/v20/coin_activation/enable_eth_with_tokens/)).
