...
| i\_am\_seed                     | boolean         |     ✗    |         `false`         | Runs Komodo DeFi Framework API as a seed node mode (acting as a relay for Komodo DeFi Framework API clients). Use of this mode is not reccomended on the main network (8762) as it could result in a pubkey ban if non-compliant. On alternative testing or private networks, at least one seed node is required to relay information to other Komodo DeFi Framework API clients using the same netID.               |
| is\_bootstrap\_node             | boolean         |     ✗    |         `false`         | If `true`, and `i_am_seed` is also true, KDF will act as a bootstrap node for the network.                                                                                                                                                                                                                                                                                                                           |
| is\_watcher                     | boolean         |     ✗    |         `false`         | If `true`, KDF will operate as a watcher seed nodes which may (in some cases) complete the final steps of a swap when a party goes offline.                                                                                                                                                                                                                                                                          |
...
| wallet\_password                | string          |     ✗    |           `-`           | Required if using `wallet_name`. The password use to encrypt your wallet's mnemonic phrase for local storage, and decrypt the wallet when logging in.                                                                                                                                                                                                                                                                |
