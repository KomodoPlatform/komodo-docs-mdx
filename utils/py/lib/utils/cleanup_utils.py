...
from .file_utils import normalize_file_path, ensure_directory_exists
...
class GeneratedFilesCleaner:
    ...
    def __init__(self, workspace_root: Union[str, Path], verbose: bool = True):
        """
        ...
        """
        self.workspace_root = normalize_file_path(workspace_root)
        ...
        self.FILE_CATEGORIES = {
            'openapi': {
                'patterns': [
                    'openapi/paths/**/*.yaml',
                    'openapi/paths/**/*.yml', 
                    'openapi/components/schemas/Generated.yaml',  # Auto-generated schemas
                    'openapi/components/schemas/Generated.yml',
                    'openapi/temp/**/*',  # Temporary OpenAPI files
                ],
                'description': 'OpenAPI specification files',
                'preserve_dirs': ['openapi/components/schemas'],  # Keep manual schemas
                'preserve_files': [
            ...
            'method_mappings': {
                'patterns': [
                    'utils/py/data/method_*.json',
                    'utils/py/data/mapping_*.json',
                    'utils/py/data/generated_*.json',
                ],
                'description': 'Method mapping and metadata files',
                'preserve_dirs': [],
                'preserve_files': []
            },
            'temp_files': {
    ...
    def _get_files_for_category(self, category: str, max_age_days: Optional[int] = None) -> List[Path]:
        """Get all files for a given category that match the cleanup criteria."""
        if category not in self.FILE_CATEGORIES:
            return []
        ...
        for pattern in category_info['patterns']:
            ...
            if '**' in pattern or '*' in pattern:
                ...
                if pattern_path.exists():
                    if max_age_days is not None:
                        file_age = datetime.now() - datetime.fromtimestamp(pattern_path.stat().st_mtime)
                        if file_age.days < max_age_days:
                            continue
                    
                    files_to_clean.append(pattern_path)
...
def clean_generated_files(workspace_root: Union[str, Path], 
    ...
    Returns:
        Dictionary with cleanup results for each category
    """
    cleaner = GeneratedFilesCleaner(workspace_root, verbose)
