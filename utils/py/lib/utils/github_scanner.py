...
import time
from ..scanning.repository_scanner import RepositoryFetcher
...
class GitHubScanner:
    ...
    def __init__(self):
        ...
        self.repo_fetcher = RepositoryFetcher(default_branch=self.branch, verbose=False)
    ...
    def get_dispatcher_content(self, version: str) -> Optional[str]:
        ...
        urls = self.repo_fetcher.get_repository_urls(self.branch)
        if version not in urls:
            return None
        
        return self.repo_fetcher.fetch_source_content(urls[version], version)
        ...
        for pattern in method_patterns:
            if re.search(pattern, dispatcher_content):
                return {
                    ...
                    "html_url": self.repo_fetcher.get_repository_urls(self.branch)[version],
