"""
Documentation Generator for Komodo DeFi Framework Methods

This module generates compliant MDX documentation using the template
and following all style guide requirements.
"""

import json
import re
from pathlib import Path
from typing import Dict, List, Optional, Any
from .method_analyzer import MethodAnalysis, Parameter


class DocumentationGenerator:
    """Generates MDX documentation following KDF style guidelines."""
    
    def __init__(self):
        self.template_path = None
        
    def load_template(self, template_path: Path) -> str:
        """Load the MDX template file."""
        try:
            with open(template_path, 'r') as f:
                return f.read()
        except FileNotFoundError:
            # Return a basic template if file not found
            return self.get_basic_template()
    
    def get_basic_template(self) -> str:
        """Return a basic template if the template file is not found."""
        return '''export const title = "{title}";
export const description = "{description}";

# {human_title}

## {method_name} {{{{label : '{method_name}', tag : '{api_tag}'}}}}

{method_description}

### Request Parameters

{request_table}

#### 📌 Examples

<CodeGroup title="" tag="POST" label="{method_name}" mm2MethodDecorate="true">
```json
{example_request}
```
</CodeGroup>

### Response Parameters

{response_table}

<CollapsibleSection expandedText='Hide Response' collapsedText='Show Response'>

#### Response

```json
{example_response}
```
</CollapsibleSection>

### Error Types

{error_table}

<CollapsibleSection expandedText="Hide Error Responses" collapsedText="Show Error Responses">

{error_responses}

</CollapsibleSection>

<Note>
View the source code at: https://github.com/KomodoPlatform/komodo-defi-framework/blob/dev/{source_file}
</Note>'''
    
    def generate_parameter_table(self, parameters: List[Parameter], include_default: bool = True) -> str:
        """Generate a parameter table in markdown format."""
        if not parameters:
            return "No parameters required."
        
        # Determine if we need the Default column
        has_defaults = any(param.default_value is not None or not param.required for param in parameters)
        needs_default_column = include_default and has_defaults
        
        # Table headers
        if needs_default_column:
            headers = "| Parameter | Type | Required | Default | Description |"
            separator = "| --------- | ---- | :------: | :-----: | ----------- |"
        else:
            headers = "| Parameter | Type | Required | Description |"
            separator = "| --------- | ---- | :------: | ----------- |"
        
        table_lines = [headers, separator]
        
        # Sort parameters: required first, then optional
        sorted_params = sorted(parameters, key=lambda p: (not p.required, p.name))
        
        for param in sorted_params:
            # Format type
            param_type = param.param_type
            if param.is_array:
                if param_type != 'array':
                    param_type = f"array of {param_type}s"
                else:
                    param_type = "array"
            
            # Format required/optional
            required_mark = "✓" if param.required else "✗"
            
            # Format default value
            if needs_default_column:
                if param.default_value:
                    default_val = f"`{param.default_value}`"
                elif not param.required and param.param_type == 'boolean':
                    default_val = "`false`"
                elif not param.required:
                    default_val = "-"
                else:
                    default_val = "-"
                
                row = f"| {param.name} | {param_type} | {required_mark} | {default_val} | {param.description} |"
            else:
                row = f"| {param.name} | {param_type} | {required_mark} | {param.description} |"
            
            table_lines.append(row)
        
        return "\n".join(table_lines)
    
    def generate_error_table(self, error_types: List[str]) -> str:
        """Generate error types table."""
        if not error_types:
            return "No specific error types documented."
        
        headers = "| Parameter | Type | Description |"
        separator = "| --------- | ---- | ----------- |"
        table_lines = [headers, separator]
        
        # Common error descriptions
        error_descriptions = {
            "NoSuchCoin": "The specified coin was not found or is not activated yet",
            "InvalidParam": "One or more parameters are invalid or missing",
            "InternalError": "The request failed due to a Komodo DeFi Framework API internal error",
            "Transport": "The request failed due to a network error",
            "Timeout": "The operation timed out",
            "InvalidAddress": "The specified address is not valid",
            "InsufficientBalance": "Insufficient balance for the operation",
            "CoinIsNotActive": "The specified coin is not currently active",
            "RpcError": "An RPC error occurred during the operation"
        }
        
        for error_type in sorted(error_types):
            description = error_descriptions.get(error_type, f"Error related to {error_type.lower()} operation")
            row = f"| {error_type} | string | {description} |"
            table_lines.append(row)
        
        return "\n".join(table_lines)
    
    def generate_error_responses(self, error_types: List[str]) -> str:
        """Generate example error responses."""
        if not error_types:
            return "##### Generic Error\n\n```json\n{\n  \"mmrpc\": \"2.0\",\n  \"error\": \"Internal error\",\n  \"error_path\": \"method\",\n  \"error_trace\": \"method:123]\",\n  \"error_type\": \"InternalError\",\n  \"id\": null\n}\n```"
        
        error_examples = []
        
        for i, error_type in enumerate(error_types[:3]):  # Limit to 3 examples
            if error_type == "NoSuchCoin":
                example = {
                    "mmrpc": "2.0",
                    "error": "No such coin BTC",
                    "error_path": "coin_activation",
                    "error_trace": "coin_activation:123]",
                    "error_type": "NoSuchCoin",
                    "error_data": {"coin": "BTC"},
                    "id": i
                }
                title = "Error Response (No Such Coin)"
            elif error_type == "InvalidParam":
                example = {
                    "mmrpc": "2.0",
                    "error": "Invalid parameter: missing required field",
                    "error_path": "method",
                    "error_trace": "method:456]",
                    "error_type": "InvalidParam",
                    "id": i
                }
                title = "Error Response (Invalid Parameter)"
            elif error_type == "InternalError":
                example = {
                    "mmrpc": "2.0",
                    "error": "Internal error occurred",
                    "error_path": "method",
                    "error_trace": "method:789]",
                    "error_type": "InternalError",
                    "id": i
                }
                title = "Error Response (Internal Error)"
            else:
                example = {
                    "mmrpc": "2.0",
                    "error": f"{error_type} error occurred",
                    "error_path": "method",
                    "error_trace": "method:999]",
                    "error_type": error_type,
                    "id": i
                }
                title = f"Error Response ({error_type})"
            
            error_json = json.dumps(example, indent=2)
            error_examples.append(f"##### {title}\n\n```json\n{error_json}\n```")
        
        return "\n\n".join(error_examples)
    
    def generate_example_response(self, response_params: List[Parameter]) -> str:
        """Generate an example response based on response parameters."""
        if not response_params:
            return '{\n  "mmrpc": "2.0",\n  "result": "success",\n  "id": null\n}'
        
        result = {}
        for param in response_params:
            if param.param_type == 'string':
                if 'coin' in param.name.lower():
                    result[param.name] = "BTC"
                elif 'status' in param.name.lower():
                    result[param.name] = "Ok"
                elif 'hash' in param.name.lower():
                    result[param.name] = "0x1234567890abcdef1234567890abcdef12345678"
                else:
                    result[param.name] = "example_value"
            elif param.param_type == 'integer':
                result[param.name] = 123456
            elif param.param_type == 'float':
                result[param.name] = 1.23456789
            elif param.param_type == 'boolean':
                result[param.name] = True
            elif param.param_type == 'array':
                result[param.name] = ["item1", "item2"]
            elif param.param_type == 'object':
                result[param.name] = {"key": "value"}
            else:
                result[param.name] = "example_value"
        
        response = {
            "mmrpc": "2.0",
            "result": result,
            "id": None
        }
        
        return json.dumps(response, indent=2)
    
    def generate_export_title(self, method_name: str, human_title: str) -> str:
        """Generate the export title following style guide rules."""
        return f"Komodo DeFi Framework Method: {human_title}"
    
    def generate_export_description(self, method_name: str, description: str) -> str:
        """Generate the export description."""
        # Clean up the description for export
        clean_desc = description.replace(f"The `{method_name}` method", f"The {method_name} method")
        clean_desc = clean_desc.replace("provides functionality for", "for")
        return f"{clean_desc.rstrip('.')} in the Komodo DeFi Framework API."
    
    def get_source_file_reference(self, source_files: List[str]) -> str:
        """Get the first relevant source file for the Note section."""
        if not source_files:
            return "mm2src/"  # Generic fallback
        
        # Prefer implementation files over test files
        impl_files = [f for f in source_files if 'test' not in f.lower() and f.endswith('.rs')]
        if impl_files:
            return impl_files[0]
        
        return source_files[0] if source_files else "mm2src/"
    
    def generate_documentation(
        self,
        method_name: str,
        human_title: str,
        description: str,
        api_tag: str,
        analysis: MethodAnalysis,
        template_path: Path
    ) -> str:
        """Generate complete MDX documentation."""
        
        # Load template
        template = self.load_template(template_path)
        
        # Generate export title and description
        export_title = self.generate_export_title(method_name, human_title)
        export_description = self.generate_export_description(method_name, description)
        
        # Generate parameter tables
        request_table = self.generate_parameter_table(analysis.request_params, include_default=True)
        response_table = self.generate_parameter_table(analysis.response_params, include_default=False)
        
        # Generate error documentation
        error_table = self.generate_error_table(analysis.error_types)
        error_responses = self.generate_error_responses(analysis.error_types)
        
        # Generate examples
        example_request = json.dumps(analysis.examples[0] if analysis.examples else {}, indent=2)
        example_response = self.generate_example_response(analysis.response_params)
        
        # Get source file reference
        source_file = self.get_source_file_reference(analysis.source_files)
        
        # Replace template placeholders
        content = template.format(
            title=export_title,
            description=export_description,
            human_title=human_title,
            method_name=method_name,
            api_tag=api_tag,
            method_description=description,
            request_table=request_table,
            response_table=response_table,
            error_table=error_table,
            error_responses=error_responses,
            example_request=example_request,
            example_response=example_response,
            source_file=source_file
        )
        
        # Clean up any formatting issues
        content = self.clean_up_formatting(content)
        
        return content
    
    def clean_up_formatting(self, content: str) -> str:
        """Clean up formatting issues in the generated content."""
        # Remove extra blank lines
        content = re.sub(r'\n{3,}', '\n\n', content)
        
        # Ensure proper spacing around headers
        content = re.sub(r'\n(#+)\s*([^\n]+)\n', r'\n\n\1 \2\n\n', content)
        
        # Clean up table formatting
        content = re.sub(r'\|\s*\|\s*', '| ', content)
        content = re.sub(r'\s*\|\s*\n', ' |\n', content)
        
        # Ensure CodeGroup and CollapsibleSection have proper spacing
        content = re.sub(r'\n(<CodeGroup)', r'\n\n\1', content)
        content = re.sub(r'\n(<CollapsibleSection)', r'\n\n\1', content)
        content = re.sub(r'(</CollapsibleSection>)\n', r'\1\n\n', content)
        
        return content.strip() + '\n' 