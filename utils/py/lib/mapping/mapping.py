...
from ..scanning.unified_scanners import UnifiedScanner
...
class MethodMapper:
    """
    Unified method mapper that consolidates all file type scanning.
    
    Uses UnifiedScanner to eliminate duplicate scanning implementations.
    """
    ...
    def __init__(self, base_path: str = ".", verbose: bool = True):
        ...
        # Configure directories for UnifiedScanner
        ...
        # Initialize components - use UnifiedScanner for all file scanning
        self.normalizer = MethodNameNormalizer()
        self.unified_scanner = UnifiedScanner(self.base_directories, verbose)
        self.openapi_manager = OpenAPIManager(self.main_openapi_file, 
                                            {'v1': self.base_directories['yaml_v1'], 
                                             'v2': self.base_directories['yaml_v2']}, verbose)
    ...
    def scan_mdx_files(self) -> Dict[str, Dict[str, str]]:
        ...
        # Use UnifiedScanner for both v1 and v2
        v1_results = self.unified_scanner.scan_mdx_files('v1')
        v2_results = self.unified_scanner.scan_mdx_files('v2')
    ...
    def scan_yaml_files(self) -> Dict[str, Dict[str, str]]:
        ...
        # Use UnifiedScanner for both v1 and v2
        v1_results = self.unified_scanner.scan_yaml_files('v1')
        v2_results = self.unified_scanner.scan_yaml_files('v2')
    ...
    def scan_json_examples(self) -> Dict[str, Dict[str, Tuple[str, int]]]:
        ...
        # Use UnifiedScanner and convert format to match expected output
        v1_examples = self.unified_scanner.scan_json_examples('v1')
        v2_examples = self.unified_scanner.scan_json_examples('v2')
        
        # Convert from List[Dict] format to (path, count) format
        def convert_examples_format(examples_dict):
            converted = {}
            for method, examples_list in examples_dict.items():
                if examples_list:
                    # Use the first example's path info and count total examples
                    first_example = examples_list[0]
                    relative_path = first_example.get('relative_path', '')
                    # Extract directory path (remove filename)
                    if '/' in relative_path:
                        dir_path = '/'.join(relative_path.split('/')[:-1])
                    else:
                        dir_path = relative_path
                    converted[method] = (dir_path, len(examples_list))
            return converted
        
        return {
            "v1": convert_examples_format(v1_examples),
            "v2": convert_examples_format(v2_examples)
        }
    ...
    def debug_method_matching(self, method_name: str, version: str = "v2") -> None:
        ...
        yaml_mappings = self.scan_yaml_files()
        example_mappings = self.scan_json_examples()
        variations = self.normalizer.generate_method_variations(method_name)
    ...
    def _find_method_mappings(self, method_name: str, unified: Dict) -> List[Tuple[str, MethodMapping]]:
        ...
        for version in ["v1", "v2"]:
            for m, mapping in unified[version].items():
                if (m == method_name or 
                    m in self.normalizer.generate_method_variations(method_name) or 
                    method_name in self.normalizer.generate_method_variations(m)):
    ...
    def _normalize_method_name(self, method_name: str) -> List[str]:
        ...
        return self.normalizer.generate_method_variations(method_name)
