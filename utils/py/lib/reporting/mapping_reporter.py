...
from ..managers.method_mapping import MethodMapping
...
class MappingReporter(BaseReporter):
    ...
    def generate_comparison_report(self, comparison: Dict[str, Dict[str, List[str]]], 
        ...
        """
        Generate a human-readable comparison report for repository vs documentation.
        ...
        """
        lines = []
        lines.append("# KDF Repository vs Documentation Comparison Report")
        lines.append(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        lines.append("")
        
        for version, results in comparison.items():
            lines.append(f"## {version.upper()} API")
            lines.append("")
            
            repo_total = results["repo_total"]
            docs_total = results["docs_total"]  
            common_total = results["common_total"]
            
            lines.append(f"- **Repository Methods**: {repo_total}")
            lines.append(f"- **Documentation Methods**: {docs_total}")
            lines.append(f"- **Common Methods**: {common_total}")
            lines.append("")
            
            if results["missing_in_docs"]:
                lines.append(f"### Missing in Documentation ({len(results['missing_in_docs'])} methods)")
                for method in results["missing_in_docs"]:
                    lines.append(f"- `{method}`")
                lines.append("")
            
            if results["missing_in_repo"]:
                lines.append(f"### Missing in Repository ({len(results['missing_in_repo'])} methods)")
                for method in results["missing_in_repo"]:
                    # Try to find the MDX file path for this method
                    mdx_path = self._find_mdx_path_for_method(method, version, method_mappings)
                    if mdx_path:
                        lines.append(f"- `{method}` → {mdx_path}")
                    else:
                        lines.append(f"- `{method}`")
                lines.append("")
            ...
            # Coverage percentage
            if repo_total > 0:
                coverage = (common_total / repo_total) * 100
    ...
    def _find_mdx_path_for_method(self, method: str, version: str, 
        ...
        """Find the MDX file path for a given method."""
        if not method_mappings or version not in method_mappings:
            return None
        
        # Try direct lookup
        if method in method_mappings[version]:
            mapping = method_mappings[version][method]
            if mapping.has_mdx and mapping.mdx_path:
                return mapping.mdx_path
        
        # Try normalized lookup (in case there are slight variations)
        from ..utils.string_utils import normalize_method_name
        normalized_method = normalize_method_name(method)
        
        for stored_method, mapping in method_mappings[version].items():
            if normalize_method_name(stored_method) == normalized_method:
                if mapping.has_mdx and mapping.mdx_path:
                    return mapping.mdx_path
